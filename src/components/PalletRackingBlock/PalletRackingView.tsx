...
const PalletRackingView = () => {
  ...
  // #region GYRO/MOUSE TILT
  useEffect(() => {
    if (!group || !viewWrapperRef.current) return;

    let isMouseOver = false;

    const handleMouseMove = (e: MouseEvent) => {
      const rect = viewWrapperRef.current!.getBoundingClientRect();
      isMouseOver =
        e.clientX >= rect.left &&
      ...
      if (isMouseOver) {
        ...
        group.rotation.y = (x - 0.5) * 0.1;
        group.rotation.x = (y - 0.5) * 0.1;
      } else {
        group.rotation.y = 0;
        group.rotation.x = 0;
      }
    ...
    const handleOrientation = (e: DeviceOrientationEvent) => {
      ...
      const beta = e.beta || 0;
      group.rotation.y = (gamma / 90) * 0.4;
      group.rotation.x = (beta / 180 - 0.25) * 0.4;
    };

    // Request permission for iOS devices only after user interaction
    ...
    // Add touch/click handler to request permission
  ...
  }, [group, viewWrapperRef.current, orientationPermissionRequested]);
  // #endregion
...
export default PalletRackingView;
